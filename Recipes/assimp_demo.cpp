...
// Load a 3D model from a .glft2 file
// Not much to see in shaders and base config
// GOTO line 119
...
int main( void )
{
	...
	// First request library to convert any geometric primitive to triangles
	...
	// Convert the loaded 3D scene into a data format that we can use to upload the model
	// into OpenGL
	...
	for (unsigned int i = 0; i != mesh->mNumFaces; i++)
	{
		...
		// Flatten all the indices and store only vertex position
		// This is for sake of example simplicity
		for (int j = 0; j != 3; j++)
		{
	...
	// Deallocate the scene pointer and upload content of positions
	GLuint VAO;
	glCreateVertexArrays(1, &VAO);
	glBindVertexArray(VAO);
	...
	// Save the number of vertices that will then be used in the main loop
