...
// This code gives an example of how to work with MeshOptimiser
// 1- Load with assimp
// 2 - Generate a remap table
// 3 - Allocate Space and generate new vtx/idx buffers
// 4 - Use GPU cache to reuse transformed vertices 

using glm::mat4;
using glm::vec3;

// Shaders are interesting here, use of barycentric coordinates for wireframe rendering
// Via use of a geometry shader
// Baricentric coordinates are then used to discriminate colors in the fragment shader
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(1024, 768, "Example", nullptr, nullptr);
	...
	// Load mesh via Assimp, preserve the existing vertices
	const aiScene* scene = aiImportFile("data/rubber_duck/scene.gltf", aiProcess_Triangulate);

	if (!scene->HasMeshes() || !scene)
	{
	...
	std::vector<unsigned int> indicesLod;
	{
		...
		// Initiate vector at proper size
		std::vector<unsigned int> remap(indices.size());

		// Generates a vertex remap of vtx and idx buffer and returns number of UNIQUE vertices
		// Remap table is generated on binary equivalence 
		...
		// We can now allocate proper space for remapped vtx/idx buffers
		...
		// It is possible to use meshOpt algos to optimise buffers even further
		// GPUs can reuse small transformed vertice buffers via built-in cache
		// Touching index data will allow to maximize locality of vtx refs
		meshopt_optimizeVertexCache(remappedIndices.data(), remappedIndices.data(),
			indices.size(), vertexCount);

		// Usually, each fragment is run through a depth test first to computer final color
		// Lets reduce number of fragment invocations and reduce pixel overdraw in a mesh
		...
		// Once the the overdraw of pixels is reduced, the vertex buffer access pattern can still be
		// optimized for memory efficiency. The GPU has to fetch specified vertex attributes from the
		//vertex buffer and pass this data into the vertex shader. To speed up this fetch,
		// a memory cache is used, which means optimizing the locality of vertex buffer access is very important.
		...
		// Finally MeshOpt allows to simplify the mesh LOD mesh
		const float threshold = 0.2f;
		const size_t target_index_count = size_t(remappedIndices.size() * threshold);
		const float target_error = 1e-2f;

		// Can generate several LOD by changing threshold value
		...
		indicesLod.resize(meshopt_simplify(&indicesLod[0], remappedIndices.data(), remappedIndices.size(), 
			&remappedVertices[0].x, vertexCount, sizeof(vec3), target_index_count, target_error));
		
		// Copy Back into original !!! This is only for the demo !!!
		indices = remappedIndices;
		positions = remappedVertices;
	}

	// Store Vertex and Index data into a single buffer
